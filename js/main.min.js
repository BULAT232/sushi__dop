...
"use strict";function updateImage(){console.log(this.files),this.files&&this.files.length?(preview.src=window.URL.createObjectURL(this.files[0]),preview.setAttribute("height","100%")):preview.setAttribute()}function highlightDropZone(e){e.preventDefault(),this.classList.add("drop")}function unHighlightDropZone(e){e.preventDefault(),this.classList.remove("drop")}var dropField,input=document.getElementById("avatar"),preview=document.getElementById("preview"),dropFiles=document.getElementsByClassName("avatar-field");dropFiles&&dropFiles.length&&((dropField=dropFiles[0]).addEventListener("dragover",highlightDropZone),dropField.addEventListener("dragenter",highlightDropZone),dropField.addEventListener("dragleave",unHighlightDropZone),dropField.addEventListener("drop",function(e){var t=e.dataTransfer;unHighlightDropZone.call(dropField,e),updateImage.call(t)})),input.addEventListener("change",updateImage);
